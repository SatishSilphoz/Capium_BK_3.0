...
public void Navigate_to_bookkeeping_module() {
    	...
    	try {
    	    ...
    	    if (!clicked) {
    	        try {
    	            if (loginLocators.C_icon_inside_modules != null && loginLocators.C_icon_inside_modules.isDisplayed()) {
    	                loginLocators.C_icon_inside_modules.click();
    	                Thread.sleep(500); // Small delay to allow the submenu to open
 
    	                if (loginLocators.Bookkeeping_Module != null && loginLocators.Bookkeeping_Module.isDisplayed()) {
